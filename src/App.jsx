...
export default function App() {
  ...
  const handleTrade = async (data) => {
    ...
    if (data.type === 'buy') {
      ...
      // Profit contable (solo referencia)
      ...
      // V3.7: Convertir gasto a USDT usando la tasa del momento (AvgPrice)
    ...
    // Guardar AvgPrice actual para referencia histórica
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-slate-400 mb-8 max-w-xs">Terminal V3.7 - Stats Pro</p>
        <button 
...
// --- MÓDULO DE DASHBOARD (Resumen Hoy) ---
function Dashboard({ transactions, inventory, onDelete }) {
  // Lógica de Replay para "Hoy" (Start of Day Equity vs Now)
  const todayMetrics = useMemo(() => {
    const startOfDay = new Date();
    startOfDay.setHours(0,0,0,0);
    const startTime = startOfDay.getTime();

    // 1. Filtrar txs de hoy
    const todays = transactions.filter(t => {
      const time = t.createdAt ? t.createdAt.seconds * 1000 : Date.now();
      return time > startTime;
    });

    // 2. Reconstruir Patrimonio al inicio del día (Reverso)
    let startUSDT = safeNum(inventory.usdt);
    let startVES = safeNum(inventory.ves);
    // Para simplificar "Equity", usamos el AvgPrice Actual como referencia de valor
    // Esto es una aproximación válida para "Crecimiento de Inventario"
    ...
    const currentEquityUSDT = safeNum(inventory.usdt) + (safeNum(inventory.ves) / (safeNum(inventory.avgPrice) || 1));
    const startEquityUSDT = startUSDT + (startVES / (safeNum(inventory.avgPrice) || 1)); // Usamos tasa actual para valorar ambos puntos y ver crecimiento neto en tokens
    
    // Profit Real = Cambio en Patrimonio - Inyecciones de Capital hoy
    const capitalInjectionsUSDT = todays.filter(t => t.type === 'capital').reduce((acc, t) => {
        if(t.currency === 'USDT') return acc + safeNum(t.amount);
        if(t.currency === 'VES') return acc + (safeNum(t.amount) / (safeNum(inventory.avgPrice) || 1));
        return acc;
    }, 0);
    ...
    <div className="space-y-4 pb-20">
      <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Operaciones: <span className="text-white font-mono">{todayStats.count}</span></p>
        </div>
        <div className="text-right">
           <p className="text-[10px] text-slate-400 uppercase font-bold">Crecimiento Real</p>
           <p className={`text-xl font-bold ${todayStats.realGrowth >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
             {todayStats.realGrowth >= 0 ? '+' : ''}{todayStats.realGrowth.toFixed(2)} USDT
           </p>
        </div>
      </div>
...
// --- MÓDULO DE ESTADÍSTICAS PRO (V3.7) ---
function StatsModule({ transactions, inventory }) {
  const [range, setRange] = useState('day'); // day, month, all
  ...
  const metrics = useMemo(() => {
    ...
    if (range === 'month') startTime.setDate(1); // 1ro del mes
    ...
    // 2. Reconstruir Patrimonio al Inicio del Periodo (Replay Logic)
    let startUSDT = safeNum(inventory.usdt);
    let startVES = safeNum(inventory.ves);
    
    // Reversar todas las transacciones hasta llegar al inicio del periodo
    ...
    // Valorar Patrimonio (Usando AvgPrice actual como constante de valoración para simplificar crecimiento)
    ...
    // Crecimiento Neto Real = (Final - Inicial) - Lo que yo metí
    const realGrowthUSDT = (currentEquity - startEquity) - deposits;

    // 3. Gastos en USDT (Dolarizados)
    // Usamos el campo expenseUSDT si existe (V3.7+), o calculamos con avgPrice actual
      ...
      <div className="flex bg-slate-800 p-1 rounded-lg justify-center">
         {['day', 'month', 'all'].map(r => (
             <button 
               key={r}
               onClick={() => setRange(r)}
               className={`flex-1 py-1 text-xs font-bold rounded capitalize transition-colors ${range === r ? 'bg-slate-700 text-white' : 'text-slate-500'}`}
             >
